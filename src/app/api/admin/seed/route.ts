...
export async function POST() {
  try {
    ...
    if (
      ...
      !cleaningServices.length ||
      !paintingServices.length ||
      !movingService
    ) {
      return NextResponse.json(
        { error: "Services not seeded yet. Run prisma seed first." },
        { status: 400 },
      )
    }
    ...
    const req1 = await prisma.request.upsert({
      ...
      create: {
        ...
        description:
          "The pipe under my kitchen sink has been leaking for 2 days. Need someone to fix it urgently.",
    ...
    const req3 = await prisma.request.upsert({
      ...
      create: {
        ...
        description:
          "Want to paint my living room, approx 12x15 ft. Need both walls and ceiling done.",
        location: "Milijuli, Ward 3",
        wardNo: 3,
        budget: 3000,
        urgency: "normal",
    ...
    const t1 = await prisma.transaction.create({
      data: {
        ...
        commission: Math.round(800 * reviewRate * 100) / 100,
    ...
    await prisma.transaction.create({
      data: {
        ...
        commission: Math.round(2500 * 0.05 * 100) / 100,
        commissionRate: 0.05,
        taskerId: tasker2.id,
        createdAt: new Date(Date.now() - 17 * 24 * 60 * 60 * 1000),
      },
      ...
      await prisma.testimonial.createMany({
        data: [
          ...
          },
          {
            ...
            role: "Business Owner",
            content:
              "We use NepalSewa for office cleaning and IT support. Reliable, professional, and affordable.",
            rating: 5,
            sortOrder: 3,
          },
          {
            ...
            role: "Teacher",
            content:
              "Found a great math tutor for my son through NepalSewa. His grades have improved significantly.",
            rating: 4,
            sortOrder: 4,
          },
        ],
    ...
    return NextResponse.json({
      success: true,
      message:
        "Sample data seeded successfully. Login: ram@example.com / sita@example.com (password: password123), Taskers: hari@example.com / gita@example.com",
    })
  } catch {
